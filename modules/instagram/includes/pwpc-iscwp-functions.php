 ...
 * @since 1.0
 */
function pwpcl_iscwp_nice_number($n) {
 ...
 * @since 1.0
 */
function pwpcl_iscwp_format_number($number) {  
    if($number >= 1000) {
 ...
 * @since 1.0
 */
function pwpcl_iscwp_insta_request( $api_url, $args = array() ) {
 ...
 * @since 1.0
 */
function pwpcl_iscwp_get_user_media( $username, $limit = 60, $cache = true ) {
	...
	$transient_key 		= "pwpc_iscwp_media_{$username}";
		...
		if( isset($response_matches[1]) ) {
			...
			if ( $page_data || !empty($page_data['entry_data']['ProfilePage'][0]['graphql']['user']) ) {
				...
				if ( !$user_data['is_private'] ) {
					...
					$query_res = pwpcl_iscwp_insta_request( $api_url, $api_args );*/

					// If we are getting the data
					//if( !empty($query_res['body']) ) {

						// Only store keys to remove stored cache in case
						$cache_transient 			= get_option( 'pwpc_iscwp_cache_transient', array() );
						$cache_transient[$username] = $transient_key;
						$cache_transient 			= array_unique($cache_transient);
						update_option( 'pwpc_iscwp_cache_transient', $cache_transient );

						$result_data['iscwp_data'] 			= $user_data;
						$result_data['iscwp_user_data'] 	= pwpcl_iscwp_get_user_details( $user_data ); // Getting user details
						set_transient( $transient_key, json_encode($result_data), 259200 );
					//}
				}
			}
		}
 ...
 * @since 1.0
 */
function pwpcl_iscwp_user_media_data( $username, $media_shortcode = false ) {
 ...
 * @since 1.0
 */
function pwpcl_iscwp_insta_image_data( $media_data = array() ) {
 ...
 * @since 1.0
 */
function pwpcl_iscwp_get_user_details( $user_details = '' ) {
...
/**
 * Function to get image size according to grid
 * 
 * @subpackage Instagram
 * @since 1.2
 */
function pwpcl_insta_img_size( $grid = '' ) {
 ...
 * @since 1.0
 */
function pwpcl_iscwp_resize_insta_image($url, $width = false, $height = false) {
